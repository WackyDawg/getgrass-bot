  ...
  async connectToProxy(proxy, userID) {
    ...
    try {
      ...
      ws.on('open', async () => {
        ...
        // Save the successfully connected proxy here
        await this.saveConnectedProxy(formattedProxy, proxyInfo);
...
async saveConnectedProxy(proxy, proxyInfo) {
    ...
    try {
      ...
      //await fs.appendFile('connectedProxies.json', JSON.stringify(data) + '\n');
      console.log(`Proxy ${proxy} saved successfully.`);
    } catch (error) {
      console.error(`Failed to save proxy ${proxy}: ${error.message}`.red);
    }
}
